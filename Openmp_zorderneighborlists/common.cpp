...
void make_cell_map(){
cell_map = (int*)malloc(cells_per_side*cells_per_side*sizeof(int));
...
for(int i =0; i<cells_per_side; i++){
   for(int j =0; j<cells_per_side; j++){
	printf("%d-",i+j*cells_per_side);	
	printf("%d\t",cell_map[i+j*cells_per_side]);	
}
printf("\n");
}
...
for(int i =0; i<cells_per_side; i++){
   for(int j =0; j<cells_per_side; j++){
	printf("%d ",cell_map[j+i*cells_per_side]);	
}
printf("\n");
}
*/
...
int get_cell_map(int i){
int x = cell_map[i];

return x;
}

void sort_particles(cell *c, particle_t *p){
int index = 0;
for( int i =0; i< cells_per_side*cells_per_side; i++){
	for(int k =0; k<MAX_PARTICLES_PER_CELL; k++){
	  if(c[i].particles[k]==-1) break;
	  int next = c[i].particles[k];
	  particle_t temp = p[index];
	  p[index] = p[next];
	  p[next] = temp;
	  index++;	
	}
...
for(int i=0; i< cells_per_side;i++){
	...
	if(j+1==cells_per_side&&i+1==cells_per_side){
		c[cell_map[i+j*cells_per_side]].E = NULL;
		c[cell_map[i+j*cells_per_side]].NE = NULL;
		c[cell_map[i+j*cells_per_side]].SE = NULL;
		c[cell_map[i+j*cells_per_side]].S = NULL;
		c[cell_map[i+j*cells_per_side]].SW = NULL;
		c[cell_map[i+j*cells_per_side]].W = cell_map[i+(j-1)*cells_per_side];
//		printf("i,j: %d,%d.new i,j: %d,%d\n",i+j*cells_per_side,i+(j-1)*cells_per_side,cell_map[i+j*cells_per_side],cell_map[i+(j-1)*cells_per_side]);
	//	printf("i,j: %d,%d.\n",i+j*cells_per_side,i+(j-1)*cells_per_side);
		c[cell_map[i+j*cells_per_side]].NW = cell_map[i-1+(j-1)*cells_per_side];
		c[cell_map[i+j*cells_per_side]].N = cell_map[i-1+j*cells_per_side];
	}else if(j+1==cells_per_side&&i==0){
		c[cell_map[i+j*cells_per_side]].E = NULL;
		c[cell_map[i+j*cells_per_side]].NE = NULL;
		c[cell_map[i+j*cells_per_side]].SE = NULL;
		c[cell_map[i+j*cells_per_side]].S = cell_map[i+1+j*cells_per_side];
		c[cell_map[i+j*cells_per_side]].SW = cell_map[i+1+(j-1)*cells_per_side];
		c[cell_map[i+j*cells_per_side]].W = cell_map[i+(j-1)*cells_per_side];
		c[cell_map[i+j*cells_per_side]].NW = NULL;
		c[cell_map[i+j*cells_per_side]].N = NULL;
	}else if(j==0&&i+1==cells_per_side){
		c[cell_map[i+j*cells_per_side]].E = cell_map[i+(j+1)*cells_per_side];
		c[cell_map[i+j*cells_per_side]].NE = cell_map[i-1+(j+1)*cells_per_side];
		c[cell_map[i+j*cells_per_side]].SE = NULL;
		c[cell_map[i+j*cells_per_side]].S = NULL;
		c[cell_map[i+j*cells_per_side]].SW = NULL;
		c[cell_map[i+j*cells_per_side]].W = NULL;
		c[cell_map[i+j*cells_per_side]].NW = NULL;
		c[cell_map[i+j*cells_per_side]].N = cell_map[i-1+j*cells_per_side];
	}else if(j==0&&i==0){
		c[cell_map[i+j*cells_per_side]].E = cell_map[i+(j+1)*cells_per_side];
		c[cell_map[i+j*cells_per_side]].NE = NULL;
		c[cell_map[i+j*cells_per_side]].SE = cell_map[i+1+(j+1)*cells_per_side];
		c[cell_map[i+j*cells_per_side]].S = cell_map[i+1+j*cells_per_side];
		c[cell_map[i+j*cells_per_side]].SW = NULL;
		c[cell_map[i+j*cells_per_side]].W = NULL;
		c[cell_map[i+j*cells_per_side]].NW = NULL;
...
// Add the particle numbers to the corresponding bins.
	...
	int ycell = (p[i].y/size) *(cells_per_side);
		for(int k=0;k<MAX_PARTICLES_PER_CELL;k++){
		   if(c[cell_map[xcell+ycell*cells_per_side]].particles[k]==-1){
			c[cell_map[xcell+ycell*cells_per_side]].particles[k]=i;
			break;
		   }	
		}	
	}
...
void update_bins_2(cell *c, int nParticles, particle_t *p){
/*
	for(int i=0;i<cells_per_side*cells_per_side;i++){
		for (int k =0; k<MAX_PARTICLES_PER_CELL;k++){
		   c[i].particles[k] = -1;
...
void compute_cell(int BCL, cell *cells, particle_t *particles, double &dmin, double &davg, int &navg){  
	...
	//	particles[cells[BCL].particles[k]].ax = particles[cells[BCL].particles[k]].ay =0; 
