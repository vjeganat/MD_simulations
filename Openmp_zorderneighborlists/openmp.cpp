...
//	printf("simulation box size: %f, cutoff radius: %f, number of bins: %d, %d\n",size,cutoff,cells_per_side,cells_per_side*cells_per_side);
	...
	dmin = 1.0;
        ...
        //
//#pragma omp single     
//if((step+1)%200==0) sort_particles(cells,particles);
...
//	update_bins(cells,n,particles); 
	...
	#pragma omp master
	update_bins_2(cells,n,particles); 
	/*
	#pragma omp single
	{	
	for(int i =0;i<n;i++){
	...
	int ycell = (particles[i].y/size) *(cells_per_side);
		for(int k=0;k<MAX_PARTICLES_PER_CELL;k++){
		   if(cells[get_cell_map(xcell+ycell*cells_per_side)].particles[k]==-1){
			cells[get_cell_map(xcell+ycell*cells_per_side)].particles[k]=i;
			break;
		   }	
		}
	}
